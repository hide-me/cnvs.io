<section class="section-padding primary-bg">
    <div class="container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="thumb-carousel text-white text-center">
                    <ul class="slides">
                        <li data-thumb="https://cdn.cnvs.io/assets/img/testimonials/ericbarnes.jpeg">
                            <div class="icon">
                                <i class="material-icons" style="font-size: 64px">format_quote</i>
                            </div>
                            <div class="content">
                                <p>For my first look at Canvas, I’m really impressed with the appearance of the admin. The installation is really straightforward, especially if you are comfortable with Laravel. If you are looking to start a blog this year, Canvas would be a good choice.</p>
                                <div class="testimonial-meta">
                                    Eric Barnes,
                                    <span>Laravel News</span>
                                </div>
                            </div>
                        </li>
                        <li data-thumb="https://cdn.cnvs.io/assets/img/testimonials/brunoskvorc.jpeg">
                            <div class="icon">
                                <i class="material-icons" style="font-size: 64px">format_quote</i>
                            </div>
                            <div class="content">
                                <p>Canvas is a new mini-CMS aimed at developers who need personal blogs. If you’re looking for an easy-to-use, easy-to-set-up, and easy-to-deploy CMS, give Canvas a go and submit extensions and pull requests to expand its already very vibrant community.</p>
                                <div class="testimonial-meta">
                                    Bruno Škvorc,
                                    <span>SitePoint PHP</span>
                                </div>
                            </div>
                        </li>
                        <li data-thumb="https://cdn.cnvs.io/assets/img/testimonials/nickbasile.jpg">
                            <div class="icon">
                                <i class="material-icons" style="font-size: 64px">format_quote</i>
                            </div>
                            <div class="content">
                                <p>Canvas rocks! I’ve tried building blogs from scratch and with other plug and play CMS’s, but nothing comes close to Canvas. Instead of worrying about grinding out the features a blog needs, you can focus on crafting a blog that tells your story. I now use Canvas exclusively for my blog development and I’ve never been happier.</p>
                                <div class="testimonial-meta">
                                    Nick Basile,
                                    <span>Metric Loop</span>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</section>