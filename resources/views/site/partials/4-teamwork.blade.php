<section class="section-padding">
    <hr>
    <div class="container">
        <div class="col-md-6">
            <h2 class="title">It Takes A Team</h2>
            <div class="section-divider"></div>
            <p>Canvas is developed by a group of contributors all over the world who donate their time and skills to create a unique blogging platform for everyone.</p>
            <p>Anyone is eligible to contribute to the project. In fact, we encourage young developers to jump in and cut their teeth on the many open source technologies implemented here.</p>
            <p>We all started somewhere. Why not start here?</p>
        </div>
        <div class="col-md-6 text-right">
            <center>
                <img class="img-responsive" src="{{ asset('assets/img/teamwork/github-repo.png') }}" alt="Github Repository">
            </center>
        </div>
    </div>
    <hr>
</section>